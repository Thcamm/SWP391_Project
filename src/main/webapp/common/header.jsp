...
import="model.user.User" %> <%@ taglib prefix="c"
  ...
  <div class="header-top d-none d-lg-block">
    <div class="container d-flex justify-content-between align-items-center">
      <div class="d-flex gap-4">
        <a
          href="https://www.google.com/maps/search/?api=1&query=757+Huỳnh+Tấn+Phát,+Phú+Thuận,+Q7"
          target="_blank"
          class="info-item"
        >
          <i class="fas fa-map-marker-alt"></i> 757 Huynh Tan Phat, Phu Thuan,
          District 7
        </a>

        <span class="info-item"
          ><i class="fas fa-clock"></i> Mon - Sat / 08.00AM - 05.00PM</span
        >
    ...
    <div class="container d-flex justify-content-between align-items-center">
      ...
      <c:choose>
        <c:when test="${sessionScope.roleCode == 'ADMIN'}">
          <c:set
            var="homeLink"
            value="${pageContext.request.contextPath}/admin/users"
          />
        </c:when>
        <c:when test="${sessionScope.roleCode == 'TECHMANAGER'}">
          <c:set
            var="homeLink"
            value="${pageContext.request.contextPath}/techmanager/home"
          />
        </c:when>
        <c:when test="${sessionScope.roleCode == 'TECHNICAL'}">
          <c:set
            var="homeLink"
            value="${pageContext.request.contextPath}/technician/home"
          />
        </c:when>
        <c:when test="${sessionScope.roleCode == 'Store Keeper'}">
          <c:set
            var="homeLink"
            value="${pageContext.request.contextPath}/inventory/dashboard"
          />
        </c:when>
        <c:when test="${sessionScope.roleCode == 'ACCOUNTANT'}">
          <c:set
            var="homeLink"
            value="${pageContext.request.contextPath}/accountant/home"
          />
        </c:when>
        <c:when test="${sessionScope.roleCode == 'CUSTOMER_SERVICE'}">
          <c:set
            var="homeLink"
            value="${pageContext.request.contextPath}/customerservice/home"
          />
        </c:when>
      </c:choose>
      <a class="navbar-brand" href="${homeLink}"> CS<span>CARSPA.VN</span> </a>
...
<div class="sidebar-menu" id="sidebarMenu">
  ...
  <nav class="sidebar-nav">
    <a
      href="${pageContext.request.contextPath}/customer/appointment-history"
      class="menu-item"
    >
      <span>Appointment List</span>
    </a>
    <a
      href="${pageContext.request.contextPath}/customer/AppointmentService"
      class="menu-item"
    >
      <span>Create Appointment</span>
    </a>
    <a
      href="${pageContext.request.contextPath}/customer/create-support-request"
      class="menu-item"
    >
    ...
    <a
      href="${pageContext.request.contextPath}/customer/workorder-list"
      class="menu-item"
    >

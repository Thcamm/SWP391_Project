...
-- Part Detail Characteristic mapping
...
CREATE TABLE `supportchannel` (
                                  ...
                                  `ContactLink` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
                                  `Description` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
...
WHERE Status = 'CLOSED'
    LIMIT 100; -- cứ chạy nhiều lần nếu cần
...
-- Kiểm tra và thêm cột progress_percentage
SET @col_exists = 0;
SELECT COUNT(*) INTO @col_exists
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_SCHEMA = 'garage_mgmt'
...
-- Tạo indexes cho TaskAssignment nếu chưa có
SET @index_exists = 0;
...
-- Kiểm tra và thêm cột request_status
SET @col_exists = 0;
SELECT COUNT(*) INTO @col_exists
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_SCHEMA = 'garage_mgmt'
...
-- Kiểm tra và thêm cột requested_at
SET @col_exists = 0;
SELECT COUNT(*) INTO @col_exists
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_SCHEMA = 'garage_mgmt'
...
-- Tạo indexes cho WorkOrderPart nếu chưa có
SET @index_exists = 0;
SELECT COUNT(*) INTO @index_exists
FROM INFORMATION_SCHEMA.STATISTICS
WHERE TABLE_SCHEMA = 'garage_mgmt'
...
ALTER TABLE PartDetail DROP FOREIGN KEY fk_pdetail_type;

-- Bước 2: Xóa cột TypeID không còn sử dụng.
ALTER TABLE PartDetail DROP COLUMN TypeID;

-- Bước 3: Thêm các cột mới cần thiết cho việc quản lý kho chi tiết.
-- Thêm SKU để định danh duy nhất, MinStock để cảnh báo tồn kho, Location cho vị trí vật lý.
ALTER TABLE PartDetail
    ADD COLUMN SKU VARCHAR(50) UNIQUE AFTER PartID,
    ADD COLUMN MinStock INT DEFAULT 10 AFTER Quantity,
    ADD COLUMN Location VARCHAR(50) AFTER UnitPrice;

-- Bước 4: Sửa đổi cột Quantity để tự động gán giá trị 0 cho các bản ghi mới nếu không được chỉ định.
ALTER TABLE PartDetail MODIFY COLUMN Quantity INT NOT NULL DEFAULT 0;

-- THÊM bảng trung gian (QUAN TRỌNG!)
CREATE TABLE PartDetail_Characteristic (
                                           id            INT AUTO_INCREMENT PRIMARY KEY,
                                           PartDetailID  INT NOT NULL,
                                           ValueID       INT NOT NULL,
                                           FOREIGN KEY (PartDetailID) REFERENCES PartDetail(PartDetailID) ON DELETE CASCADE,
                                           FOREIGN KEY (ValueID) REFERENCES CharacteristicValue(ValueID),
                                           UNIQUE KEY (PartDetailID, ValueID)
) ENGINE=InnoDB;
...
ALTER TABLE Unit ADD UNIQUE KEY name (name);

-- ========== PART (chỉ đồng bộ charset/collation) ==========
ALTER TABLE Part
    CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- BỎ index liên quan TypeID nếu có
-- ALTER TABLE PartDetail DROP INDEX ix_pdetail_type;

ALTER TABLE PartDetail
    CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
    ADD COLUMN Manufacturer VARCHAR(100) NULL AFTER Location,
    ADD COLUMN Desciption TEXT NULL AFTER Manufacturer;
...
ALTER TABLE WorkOrderDetail
    ADD COLUMN detail_status ENUM('PENDING','IN_PROGRESS','COMPLETE')
  NOT NULL DEFAULT 'PENDING',
  ADD INDEX ix_wod_detail_status (detail_status);
...
ALTER TABLE partdetail
    CHANGE COLUMN `Desciption` `Description` VARCHAR(255) NULL;
...
ALTER TABLE taskassignment
    MODIFY COLUMN Status ENUM('ASSIGNED','IN_PROGRESS','COMPLETE','DECLINED','CANCELLED')
